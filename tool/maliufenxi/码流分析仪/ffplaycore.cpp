...
static sendArg PSendArg;
SendAgrStruct *tempSendArg;
...
static AVInputFormat *file_iformat;//NULL; //"C:\\ts1.ts";//
static const char *input_filename = "G:\\迅雷下载\\The.Monkey.King.2014.1080p.BluRay.REMUX.AVC.DTS-HD.MA5.1-RARBG\\The.Monkey.King.2014.1080p.BluRay.REMUX.AVC.DTS-HD.MA5.1-RARBG.mkv";
...
int ffmfc_param_global(VideoState *is){
	...
	//输入文件的协议----------
	input_protocol.Format(L"%s",up->name);

	CString  temp;
	temp.Empty();
	temp = CStringA(input_protocol.GetBuffer());
	// dlg->m_formatprotocol.SetWindowText(input_protocol);

	//视频解码参数，有视频的时候设置
	if(video_stream!=-1){
		wxh.Format(L"%d x %d",pCodecCtx->width,pCodecCtx->height);
		//dlg->m_codecvresolution.SetWindowText(wxh);

		decoder_name.Format(L"%s",pCodecCtx->codec->long_name);
	...
	//	dlg->m_codecvname.SetWindowText(decoder_name);
		...
		framerate.Format(L"%5.2ffps",framerate_temp);
	//	dlg->m_codecvframerate.SetWindowText(framerate);

		switch(pCodecCtx->pix_fmt){
		case 0:
			pix_fmt.Format(L"YUV420P");break;
		case 1:
			pix_fmt.Format(L"YUYV422");break;
		case 2:
			pix_fmt.Format(L"RGB24");break;
		case 3:
			pix_fmt.Format(L"BGR24");break;
		case 12:
			pix_fmt.Format(L"PIX_FMT_YUVJ420P");break;	
		default:
			pix_fmt.Format(L"UNKNOWN");
		}
	//	dlg->m_codecvpixfmt.SetWindowText(pix_fmt);
	}
	//音频解码参数，有音频的时候设置
	if(audio_stream!=-1){
		decoder_name_au.Format(L"%s",pCodecCtx_au->codec->long_name);
	//	dlg->m_codecaname.SetWindowText(decoder_name_au);
		sample_rate_au.Format(L"%d",pCodecCtx_au->sample_rate);
	//	dlg->m_codecasamplerate.SetWindowText(sample_rate_au);
		channels_au.Format(L"%d",pCodecCtx_au->channels);
	//	dlg->m_codecachannels.SetWindowText(channels_au);
	}
	//显示成以k为单位
	bitrate_temp=((float)(pFormatCtx->bit_rate))/1000;
	bitrate.Format(L"%5.2fkbps",bitrate_temp);
//	dlg->m_formatbitrate.SetWindowText(bitrate);
	...
	tmm  = (tns % 3600) / 60;
	tss  = (tns % 60);
	timelong.Format(L"%02d:%02d:%02d",thh,tmm,tss);
	//dlg->m_formatduration.SetWindowText(timelong);
	//dlg->m_duration.SetWindowText(timelong);
	//输入文件的封装格式------
	input_format.Format(L"%s",pFormatCtx->iformat->long_name);
	...
	//CStringA  author,copyright,description;
	...
	*/
	...
	while(m=av_dict_get(pFormatCtx->metadata,"",m,AV_DICT_IGNORE_SUFFIX)){
		key.Format(L"%s",m->key);
		value.Format(L"%s",m->value);
		meta+=key+L"\t:"+value+L"\r\n" ;
	}
...
int ffmfc_param_vframe(VideoState *is,AVFrame *pFrame,AVPacket *packet){
	...
	//------------------------------
	f_index.Format(L"%d",vframe_index);
	//获取当前记录条数
...
//	lvitem.iItem=nIndex;
	...
	case 0:
		key_frame.Format(L"No");break;
	case 1:
		key_frame.Format(L"Yes");break;
	default:
		key_frame.Format(L"Unknown");
	}

	switch(pFrame->pict_type){
	case 0:
		pict_type.Format(L"Unknown");break;
	case 1:
		pict_type.Format(L"I");break;
	case 2:
		pict_type.Format(L"P");break;
	case 3:
		pict_type.Format(L"B");break;
	case 4:
		pict_type.Format(L"S");break;
	case 5:
		pict_type.Format(L"SI");break;
	case 6:
		pict_type.Format(L"SP");break;
	case 7:
		pict_type.Format(L"BI");break;
	default:
		pict_type.Format(L"Unknown");
	}

	reference.Format(L"%d",pFrame->reference);
	pts.Format(L"%d",pFrame->pkt_pts);
	dts.Format(L"%d",pFrame->pkt_dts);
	codednum.Format(L"%d",pFrame->coded_picture_number);
...
int ffmfc_param_aframe(VideoState *is,AVFrame *pFrame,AVPacket *packet){
	...
	//---------------
	number.Format(L"%d",aframe_index);
	//获取当前记录条数
	...
	//务必使用f_index执行Format!再赋值！
	lvitem.pszText=number.GetBuffer();
	//------------------------
	packet_size.Format(L"%d",packet->size);
	pts.Format(L"%d",packet->pts);
	dts.Format(L"%d",packet->dts);
	//---------------
	/*
...
static int video_open(VideoState *is, int force_set_video_mode)
{
	...
	if (screen && is->width == screen->w && screen->w == w
		...
		return 0;
	if (tempSendArg->dlgtype==ts)
	   screen = SDL_SetVideoMode(rect1.w, rect1.h, 0, 0);//flags
	else
		screen = SDL_SetVideoMode(w, h, 0, flags);//flags
	if (!screen) {
		AfxMessageBox(L"SDL: could not set video mode - exiting\n");
		do_exit(is);
	}
	//注意：设置视频窗口标题！
...
display:
	...
	if (show_status) {
		...
		//	dlg->m_playprogress.SetPos(pos);
			...
			tmm  = (tns % 3600) / 60;
			tss  = (tns % 60);
			currentclockstr.Format(L"%02d:%02d:%02d",thh,tmm,tss);
...
static int read_thread(void *arg)
{
	...
	VideoState *is = ((sendArg*)arg)->m_is;
	...
	//dump_format(ic, 0, filePath, 0);
	//av_dump_format(ic, 0, is->filename, 0);
	// 获取第个视频流，音频流索引
	if (tempSendArg->dlgtype == ts)//默认为TS流文件
	{
		int m_VideoStream[20] = { -1 }, videoindex = 0;
		int m_AudioStream[20] = { -1 }, Audioindex = 0;

		dlg->totalProgramNumber = ic->nb_programs;
		for (int index = 0; index < ic->nb_programs; index++)
		{
			m_VideoStream[videoindex++] = ic->programs[index]->stream_index[0];
			m_AudioStream[Audioindex++] = ic->programs[index]->stream_index[1];
		}
		st_index[AVMEDIA_TYPE_VIDEO] = m_VideoStream[((sendArg*)arg)->programN];
		st_index[AVMEDIA_TYPE_AUDIO] = m_AudioStream[((sendArg*)arg)->programN];
	}
	else
	{
	...
	if (is->video_stream < 0 && is->audio_stream < 0) {
		AfxMessageBox(L"could not open codecs");
		ret = -1;
		goto fail;
	}
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
	...
	is->read_tid = SDL_CreateThread(read_thread, &PSendArg);
	if (!is->read_tid) {
		av_free(is);
		return NULL;
	}
...
{
     tempSendArg = (SendAgrStruct*)lpParam;
	...
	//GetWindowText(dlg->m_inputurl,(LPWSTR)input_filename,MAX_URL_LENGTH);//**********************************************8
	...
	*/
	...
	//SDL_Window * pWindow = SDL_CreateWindowFrom((void *)(GetDlgItem(IDC_PIC)->GetSafeHwnd()));
	if (tempSendArg->dlgtype==ts)
	{
		dlg = tempSendArg->dlgCCTs;
		input_filename = dlg->filepath.GetBuffer();
		PSendArg.programN = dlg->programbn;
		PSendArg.isTS = dlg->isTS;////
		char name[256] = { 0 };
		CWnd* pWnd = dlg->GetDlgItem(IDC_pic1);
		...
		//ScreenToClient(dlg, &rect1);
		rect1.x = rc.left;
		rect1.y = rc.top;
		rect1.w = rc.right - rc.left;
		rect1.h = rc.bottom - rc.top;
	...
	{
		mediaMode *dlgMedia;
		dlgMedia = tempSendArg->dlgMedia;
		input_filename = dlgMedia->filepath.GetBuffer();
	...
	if (av_lockmgr_register(lockmgr)) {
		AfxMessageBox(L"Could not initialize lock manager!");
		do_exit(NULL);
	}
	...
	if (!is) {
		AfxMessageBox(L"Failed to initialize VideoState!");
		do_exit(NULL);
	}
